"""Static additive-model test for the replay conditions: does VAO == AO + VO?

For every subject this computes the difference wave

    D(t) = VAO(t) - [ AO(t) + VO(t) ]

i.e. the strict additive model. A reliable departure of D from
zero is genuine multisensory interaction. The script summarises D across the
group and reports component-level deviations in the N1 and P2 windows.
...
def analyse(experiment: str, site: tuple) -> None:
    ...
    predicted = ao + vo                        # strict additive prediction
    diff = vao - predicted                     # multisensory interaction wave
    ...
    per_subject_r2 = np.array(
        [io.r_squared(vao_site[i], pred_site[i]) for i in range(cs.n_subjects)]
    )
    per_subject_rmse = np.sqrt(
        np.mean((vao_site - pred_site).reshape(cs.n_subjects, -1) ** 2, axis=1)
    ) * UV
    ...
    for name, spec in comps.items():
        window = spec["window"]
        vao_c = io.window_mean(vao_roi, times, window) * UV
        pred_c = io.window_mean(pred_roi, times, window) * UV
        d_c = vao_c - pred_c
        t_val, p_val = stats.ttest_1samp(d_c, 0.0)
        comp_dev[name] = dict(vao=vao_c, pred=pred_c, dev=d_c, t=t_val, p=p_val)

    for i, subject in enumerate(cs.subjects):
        row = {"experiment": experiment, "subject": subject}
        for name, c in comp_dev.items():
            row[f"VAO_{name}_uV"] = c["vao"][i]
            row[f"AOplusVO_{name}_uV"] = c["pred"][i]
    ...
    # --- Console summary -----------------------------------------------------
    n = cs.n_subjects
    print(f"\n=== Static additive model  VAO ?= AO + VO   [{experiment} | {roi_label}] ===")
    ...
    for name in comp_dev:
        ...
        print(f"  {name}: VAO={c['vao'].mean():+.3f}  AO+VO={c['pred'].mean():+.3f}  "
    ...
    fig.suptitle(f"Static additive model  VAO vs AO+VO  -  {experiment}  "
                 f"({roi_label}, n={n})")

    ax_top.plot(t_ms, vao_roi.mean(0) * UV, color="tab:blue", lw=2, label="VAO (observed)")
    ax_top.plot(t_ms, pred_roi.mean(0) * UV, color="tab:red", lw=2, ls="--",
                label="AO + VO (predicted)")
